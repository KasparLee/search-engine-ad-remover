	...
	if (message.id === 'get_prefs') {
		...
		function def_google() {
			chrome.storage.local.get('google', function(prefs) {
				...
				['above', 'sponsored', 'twitter', 'r_searches', 'maps_above', 'shop_below'].forEach(function (item) {
					if (prefs[item] === undefined) {
						if (item === 'twitter') {
							prefs[item] = false
						}
						else {
		...
		function def_bing() {
			chrome.storage.local.get('bing', function(prefs) {
				...
				['above', 'below', 'right', 'products', 'r_searches', 'maps_above'].forEach(function (item) {
					if (prefs[item] === undefined) {
						prefs[item] = true
					}
				})
